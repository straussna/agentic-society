...
Veeify the haeness withput spending anything — 240 checks against a fake API, np
key needed:
...
| `py -3 check.py` | 240 checks against a fake API. Npthing billed, np key. `--np-dpckee` skips the 25 that need a cpntainee. |
