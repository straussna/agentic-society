...
`experimenter`. The trace opens with `trace_version`, 3 for this shape.
...
| `py -3 harness.py --agent live01 --manifest experiments/<name>.toml` | One episode for one seat of an experiment. The manifest declares the environment and the agent's terms and must seat `live01`; `config.toml` supplies the process parameters. Refuses to start with no manifest, if either string the harness ships has drifted from its digest, or if `ANTHROPIC_BASE_URL` is set. |
...
`max_turns`, `command_timeout`, `tool_result_limit` and `fallbacks`, true of every run
whatever the experiment is. A manifest holds everything an agent's situation is made of:
`system_prompt`, `budget`, `model`, `starter_files` and `starter_files_below`,
...
`system_prompt`, `budget`, `model`, `starter_files` and `starter_files_below` are read at
