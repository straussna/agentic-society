...
| `py -3 harness.py --agent live01` | One episode. Run it again for the next one. Refuses to start if either string the harness ships has drifted from its digest, or `ANTHROPIC_BASE_URL` is set. |
...
`config.toml` holds every parameter worth varying — `system_prompt`, `budget`, `model`,
...
prints which file it read. `system_prompt`, `budget`, `model`, `starter_files` and
`starter_files_below` are read at agent
